...
elseif flag="4" then
set rs=conn.execute("select * from links where fy_pass='Y' and fy_class='2' order by id asc")
while not rs.eof
i=i+1
if i>7 then
i=1
...
txt="项目名称："&t&"<BR>网站名称："&server.HTMLEncode(ars(2,i))&"<BR>网站网址：<a href="&server.HTMLEncode(ars(3,i))&" target=_blank>"&server.HTMLEncode(ars(3,i))&"</a><BR>网站分类："&server.HTMLEncode(ars(1,i))&"<BR>联系ＱＱ：1398668999<BR>网站简介："&server.HTMLEncode(ars(6,i))&"<BR>申请时间："&ars(8,i)&"<BR>状  态："&tt&"------<a href=?flag=1&id="&ars(0,i)&">通过</a> - <a href=?flag=2&id="&ars(0,i)&">未通过</a> - <a href=?flag=3&id="&ars(0,i)&">删除</a>"&chr(13)
